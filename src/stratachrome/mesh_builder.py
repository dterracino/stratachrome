...
def _grid_surface_faces(rows: int, cols: int) -> np.ndarray:
    ...
    return np.vstack(
        (
            np.column_stack(
                (lower_left.ravel(), lower_right.ravel(), upper_left.ravel())
            ),
            np.column_stack(
                (lower_right.ravel(), upper_right.ravel(), upper_left.ravel())
            ),
        )
    )
    ...
    for elevation_number, rank in enumerate(reducible_ranks, start=1):
        ...
        for component_number, label_index in enumerate(candidate_labels, start=1):
            ...
            reduced_face_chunks.append(
                grid_points[point_triangles][..., 0] * cols
                + grid_points[point_triangles][..., 1]
            )
...
class WatertightMeshBuilder:
    ...
    def build_mesh(
        ...
        if progress is not None:
            progress(
                f"Mesh complete: {len(vertices):,} vertices, {len(faces):,} triangles."
            )
